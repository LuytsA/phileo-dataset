...
def process_tile(
    ...
    dataset: str = 'downstream'
        Either 'downstream' of 'mini_foundation', because of naming conventions files must be retrieved differently
    ...
    timeseries_s2_raster = glob.glob(f'{folder_src}/{tile}_*[0-9].tif') if dataset=='downstream' else glob.glob(f'{folder_src}/{tile}_s2.tif')
    labels_raster = {}
    for label in LABELS:
...
def preprocess_tifs(
    ...
    create_geo_label : bool = False,
        If true, a new label is created that combines the labels 'date', 'kg' and 'coords are combined in a single label named 'geo'
    ...
    if create_geo_label:
        from utils import kg_legend
        n_kg_classes = len(kg_legend.keys())
